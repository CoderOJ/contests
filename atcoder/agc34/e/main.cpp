...
void dfs(int u, int f)
{
  ...
  auto m       = *std::max_element(vs.begin(), vs.end());
  int  oth_sum = 0;
  for (auto v : vs)
    if (v.second != m.second)
    {
      oth_sum += max[v.second] + sz[v.second];
    }
...
void solve()
{
  ...
  repa(u, n)
  {
    memcpy(sz, col, sizeof(*sz) * (n + 1));
    memset(min, 0, sizeof(min));
    memset(max, 0, sizeof(max));
    dfs(u, 0);
    if (min[u] == 0)
    {
      checkMin(ans, max[u] / 2);
    }
  }
