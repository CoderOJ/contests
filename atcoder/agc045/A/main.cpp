...
void solve() {
    ...
    repb (i,n-1,0) {
        ...
        if (s[i] == '0') {
            vector < int > add;
            for (auto j: st) add.push_back((j) ^ a[i]);
            for (auto j: add) st.insert(j);
        } else {
            vector < int > remain;
            for (auto j: st) if (st.count((j) ^ a[i])) remain.push_back(j);
            st.clear();
            for (auto j: remain) st.insert(j);
        }
    }
...
int T; scanf("%d",&T); while(T--) { try{ init(); solve();} catch(const char* s) {} }
