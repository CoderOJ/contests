...
void dfs(int u) {
    vector <int> cnt(5);
    for (int v:e[u]) cnt[color[v]]++;
    if (cnt[color[u]]>1) {
        int mn=8,id=0; repa (i,4) if (checkMin(mn,cnt[i])) id=i; color[u]=id; 
        for (int v:e[u]) if (color[v]==color[u]) dfs(v); } }

void solve() {
    repa (i,n) color[i]=1;
    repa (i,n) dfs(i);
    repa (i,n) { repa (j,4) if (color[i]!=j) printf("%d ",j); puts(""); }
}
