...
void init() {
    scanf(III,&n,&m,&k);
    rep (i,k) scanf(I,&stp[i]);
    rep (i,m) {
        int u, v; scanf(II,&u,&v); u--; v--;
        e[u].push_back(v+n); e[v+n].push_back(u);
        e[v].push_back(u+n); e[u+n].push_back(v);}
}
...
void solve() {
    ...
    rep (i,k) {
        stp[i]--;
        if (e[stp[i]].size()==0) continue;
        dis[stp[i]]=0; 
        que.push(stp[i]); } 
    while (!que.empty()) {
        int u=que.front(); que.pop();
        for (int v: e[u]) {
            if (checkMin(dis[v],dis[u]+1)) {
                que.push(v); } } }
    rep (i,n*2) if (dis[i]!=0x3f3f3f3f) t[dis[i]]++;
    rep (i,n*2) ses(t[i]); see("");
    repi (i,2,N-1) t[i] += t[i-2];
    int T; scanf(I,&T);
    while (T--) {
        int u; scanf(I,&u);u--;
        if (u==0) { printf(IN,k); continue; }
        if (u>=N) u=N-3-(u%2);
        printf(IN,t[u]); }
}
