...
struct Player {
    ...
    bool canAttack() { 
        ...
        return 0; }
    void nextReady() {
        if (vis) add(ready,minio.size(),1);
        while (minio[ready].attack==0) add(ready,minio.size(),1); }
    void operate (Player *B) {
        int Bid=ran(B->minio.size());
        minio[ready].operate(&B->minio[Bid]); vis=1;
        int Arattle=min(   minio[ready].rattle, 7-(int)   minio.size()+1);
        int Brattle=min(B->minio[Bid  ].rattle, 7-(int)B->minio.size()+1);
        if (   minio[ready].isDead()) { replace(   minio,ready,genRattle(Arattle)); add(ready,minio.size(),0); vis=0; }
        if (B->minio[Bid  ].isDead()) { replace(B->minio,Bid  ,genRattle(Brattle)); 
            if (B->ready == Bid) { B->vis=0; add(B->ready,B->minio.size(),0); }
