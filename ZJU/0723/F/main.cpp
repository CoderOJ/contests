...
void init() {
    C[0][0]=1; repa (i,M-1) { C[i][0]=1; repa (j,min(i,N-1)) C[i][j]=(C[i-1][j-1]+C[i-1][j])%MOD; }    
    see("init C complete");
    F[1][0]=1; 
    repi (i,2,N-1) { see(i); rep (j,(i-1)*i/2+1) {
        if (j<i-1) { F[i][j]=0; continue; }
        if (j==i-1){ F[i][j]=power(i,i-2); continue; }
        if (j==(i-1)*i/2) { F[i][j]=1; continue; }
        F[i][j] = C[ i*(i-1)/2 ][ j ];
        rep (x,i-1) rep (y,j+1) {
            int dec= C[i-1][x] * C[(i-x-2)*(i-x-1)/2][j] % MOD * F[x+1][j-y] % MOD;
            F[i][j] = (F[i][j] + MOD - dec) % MOD; } }  }
    see("init F complete"); }

void solve() {
    int n=sc.nextInt();
    rep (i,n*(n-1)/2+1) printf(IN,F[n][i]); }
