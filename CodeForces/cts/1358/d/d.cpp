...
void init() {
    ...
    rep (i,n*2) totSum[i+1] = totSum[i] + (a[i]+1)*a[i]/2;
}

inline int getBack(int n, int k) {
    return (n-k+1 + n) * k / 2;
}
...
void solve() {
    ...
    int l = 0;
    repi (i,n,2*n-1) {
        while  (sum[i+1] - sum[l] > x) l++;
        ses(i);see(l);
        checkMax(ans, totSum[i+1] - totSum[l] + getBack(a[l-1] , x-sum[i+1]+sum[l]));
    }
    printf(IN, ans);
}
