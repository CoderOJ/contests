...
void solve() {
    ...
    rep (i,n) {
        int ps=a[i], pm=a[i];
        repb (j,i-1,0) { checkMin(pm,a[j]); ps += pm;}
        int ss=0, sm=a[i];
        repi (j,i,n-1) { checkMin(sm,a[j]); ss += sm;}
        if (checkMax(mx,ps+ss)) { mid = i;}
    }
    int i = mid;
    int pm=a[mid];
    repb (j,i-1,0) { checkMin(pm,a[j]); ans[j] = pm;}
    int sm=a[mid];
    repi (j,i,n-1) { checkMin(sm,a[j]); ans[j] = sm;}
