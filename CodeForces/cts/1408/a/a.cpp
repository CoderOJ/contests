...
void solve() {
    memset(dp,0,sizeof(dp)); memset(fr,-1,sizeof(fr));
    dp[0][0][0] = dp[0][1][1] = dp[0][2][2] = 1;
    rep (i,n-1) rep (j,3) rep (f,3) if (dp[i][j][f]) rep (k,3) {
        if (a[i][j] != a[i+1][k]) {
            dp[i+1][k][f] = 1; fr[i+1][k][f] = j; } }
    rep (i,3) rep (j,3) {
        if (dp[n-1][i][j] && a[n-1][i] != a[0][j]) {
            vector <int> ans;
            int u = i;
            repb (i,n-1,0) { ans.push_back(a[i][u]); u = fr[i][u][j]; }
