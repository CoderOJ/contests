...
void p0() {
    set < int > st;
    rep (i,n) if (!st.count(a[i])) st.insert(a[i]);
              else iu.push_back(a[i]);
    sort (iu.begin(), iu.end());
    iu.erase( unique(iu.begin(), iu.end()), iu.end());
}

inline bool inl(int m, int t) {
    return ((m | t) == m);
}
...
void solve() {
    p0();
    rep (i,(int)iu.size()) {
        rep (j,n) if (inl(iu[i], a[j])) t[j] = 1;
    }
    int ans = 0;
    rep (i,n) if (t[i]) ans += b[i];
    printf(IN,ans);
}
