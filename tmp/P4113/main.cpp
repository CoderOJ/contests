...
int ta[N];
inline void modify(int u, int v) { u += 5;
    for (;u<N;u+=(u&(-u))) { ta[u] += v; } }
inline int query(int u) { u += 5;
    int res = 0;
    for (;u;u-=(u&(-u))) { res += ta[u]; }
    return res; }

struct Query { int l, r, id; 
    inline bool operator < (const Query& b) { return r<b.r; }
};
Query qs[N];
int n, m, c;
int a[N];
...
void preInit() { } 
void init() {
    n=sc.n(); c=sc.n(); m=sc.n();
    rep (i,n) a[i] = sc.n();
    rep (i,m) { int l=sc.n(), r=sc.n(); qs[i] = {l-1,r-1,i}; }
}
...
void solve() {
    ...
    rep (i,m) {
        while (id <= qs[i].r) {
            modify( pre[a[id]][1], -1 );
            pre[a[id]][1] = pre[a[id]][0]; pre[a[id]][0] = id; 
            modify( pre[a[id]][1], +1 );
            id++; } 
        ans[qs[i].id] = c - query( qs[i].l-1 ); }
    rep (i,m) { printf(IN, ans[i]); }
}
