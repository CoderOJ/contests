...
void preInit() { } void init() {
    n = sc.n(); type = sc.n();
	if(type==0) {
        repa (i,n) s[i]=s[i-1]+sc.n(); }
	else {
		int x=sc.n(),y=sc.n(),z=sc.n(),m,now=1;
		b[1]=sc.n();b[2]=sc.n();m=sc.n();
		for(int i=1;i<=m;i++) p[i]=sc.n(),l[i]=sc.n(),r[i]=sc.n();
...
#undef calc
    ...
    int pos = n;
    __int128_t ans = 0;
    while (pos) { ans += ((__int128_t)s[pos]-s[g[pos]])*(s[pos]-s[g[pos]]); pos = g[pos]; }
    print(ans);
}
