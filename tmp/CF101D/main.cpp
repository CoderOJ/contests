...
void preInit() { } void init() {
    n = sc.n();
    rep (i,n-1) { int u=sc.n(), v=sc.n(), w=sc.n(); e[u].push_back({v,w}); e[v].push_back({u,w}); }
}

long long sz[N], len[N];
void dfs(int u, int f) {
    sz[u] = 1; len[u] = 0;
    for (pr v: e[u]) {
        if (v.L != f) {
            dfs(v.L, u); sz[u] += sz[v.L]; len[u] += len[v.L]; }
        else { len[u] += v.R; } } }

long long tot, pos;
void dfs2(int u, int f) {
    tot += pos;
    sort(e[u].begin(), e[u].end(), [](pr a, pr b) { return len[a.L] * sz[b.L] < len[b.L] * sz[a.L]; });
    for (pr v: e[u]) if (v.L!=f) {
        pos += v.R; dfs2(v.L,u); pos += v.R; } }
