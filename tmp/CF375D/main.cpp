...
struct Query { int l,ll,r,k, id, ans; } qs[N];
int cnt[N], ans[N];
void solve() {
    rep (i,m) {
        int u=sc.n(), k=sc.n();
        qs[i] = { l[u], l[u]/B, r[u], k, i, 0 }; }
    sort( qs, qs+m, [](Query a, Query b) { return a.ll == b.ll ? a.r < b.r : a.ll < b.ll; } );
    auto add = [](int u) { cnt[u]++; ans[ cnt[u] ]++; };
    auto del = [](int u) { ans[ cnt[u] ]--; cnt[u]--; };
    int l=0, r=-1;
    rep (i,m) {
        while (r<qs[i].r) { r++; add(d[r]); }
        while (l>qs[i].l) { l--; add(d[l]); }
        while (r>qs[i].r) { del(d[r]); r--; }
        while (l<qs[i].l) { del(d[l]); l++; }
        qs[i].ans = ans[ qs[i].k ]; }
    sort(qs, qs+m, [](Query a, Query b) { return a.id < b.id; });
    rep (i,m) printf("%d\n", qs[i].ans);
}
