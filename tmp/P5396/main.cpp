...
const uint32 MOD = 167772161;
const uint32 MOD_W = 3;
...
solve()
{
  ...
  A[0] = 0;
  A[1] = 1;
  repi(i, 2, n) A[i] = A[i - 1] * Pre::Inv.inv[i];
  A = A.pow(k, k);

  Poly Fac(n+1);
  Fac[0] = 1;
  repa(i, n) Fac[i] = Fac[i - 1] * Int(i);

  Int k_fac_inv = Int(1) / Fac[k];
  rep(i, n + 1) printf("%u ", static_cast<unsigned>(A[i] * Fac[i] * k_fac_inv));
}
