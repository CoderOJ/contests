...
solve()
{
  ...
  Poly A(n + 1);
  rep(k, n + 1) A[k] = (Int(k).pow(n + 1) - Int(1)) / (Int(k) - Int(1));
  A[1] = n + 1;
  ...
  repa(i, n)
  {
    B[i] = B[i - 1] * Pre::Inv.inv[i];
    A[i] *= B[i];
  }
  repa(k, n) if (k & 1) A[k] = Int() - A[k];
  ...
  printf("%u", static_cast<unsigned>(res));
}
