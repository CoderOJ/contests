...
vector< pair<string,string> > get(string a, string b) {
    OOMap conflict;
    ...
    rep (i,len) {
        ...
        if (conflict.jury(pos.L,pos.R)) {
            ...
            pos = {"",""}; } }
    //cout << pos.L << " " << pos.R << endl;
    ans.back().L += pos.L;
    ans.back().R += pos.R;
    if (!conflict.jury(ans.back().L,ans.back().R)) {
        auto bak = ans.back(); ans.pop_back();
        ans.back().L += bak.L;
        ans.back().R += bak.R; }
    return ans;
}
...
void solve() {
    ...
    auto ans2 = get(a,b);
    // for (auto a: ans2) { cout << a.L << ' ' ; } cout << endl;
    // for (auto a: ans2) { cout << a.R << ' ' ; } cout << endl;
    for (auto& u: ans2) {
        reverse(u.L.begin(), u.L.end());
        reverse(u.R.begin(), u.R.end()); }
    reverse(ans2.begin(),ans2.end());
    //ses(ans1.size());see(ans2.size());
    auto& ans = (ans1.size()>ans2.size() ? ans1 : ans2);
    cout << ans.size() << endl;
    for (auto a: ans) { cout << a.L << ' ' ; } cout << endl;
    for (auto a: ans) { cout << a.R << ' ' ; } cout << endl;
}
