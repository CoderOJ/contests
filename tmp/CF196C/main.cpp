...
void preInit() { } void init() {
    ...
    rep (i,n) { int x=sc.nextInt(), y=sc.nextInt(); p[i] = {x,y,i+1}; }
}

void dfs0(int u, int f) {
    sz[u] = 1;
    for (int v:e[u]) if (v!=f) { dfs0(v,u); sz[u] += sz[v]; } }
int ans[N];
void dfs(int u, int f, int l, int r) {
    see(u,f,l,r);
    repi (i,l+1,r) if (p[i].y > p[l].y) { swap(p[i], p[l]); }
    ans[p[l].id] = u;
    Pt key = p[l];
    sort( p+l+1, p+r+1, [key](Pt a, Pt b) { 
        int ax = a.x-key.x, ay = a.y-key.y;
        int bx = b.x-key.x, by = b.y-key.y;
        return 1LL * ax * by < 1LL * ay * bx; });
    repi (i,l,r) { see(p[i].x, p[i].y); }
    int st = l+1;
    for (int v: e[u]) if (v!=f) { dfs(v,u,st,st+sz[v]-1); st+=sz[v]; } }

void solve() {
    dfs0(1,0);
    dfs(1,0,0,n-1);
    repa (i,n) printf("%d%c", ans[i], " \n"[i==n]);
}
