...
struct Taste {
    ...
    Taste assign(int u) {
        Taste r;
        rep (i,T) if (u&(1<<i)) r.c[i] = c[i];
        sort(r.c, r.c+T);
        return r; }
    bool operator < (const Taste& b) const {
        rep (i,T) if (c[i] != b.c[i]) return c[i] < b.c[i];
        return 0; }
};
...
void solve() {
    ...
    rep (i,n) {
        rep (b,(1<<T)) {
            Taste cur = a[i].assign(b);
            int deg = popCnt(b)%2 ? -1 : 1;
            ans += deg * mp[cur]; mp[cur]++; } }
    printf(IN,ans);
}
