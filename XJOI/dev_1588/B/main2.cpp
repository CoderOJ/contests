...
struct FTree {
    ...
    void modify(int u, int v) { u+=5;
        while (u<N+10) { a[u]++; u+=low(u); } }
    int query(int u) { u+=5;
        int res = 0;
...
int get(double d) {
    ...
    vector <double> dis(s, s+n+1);
    sort(dis.begin(), dis.end());
    rep (i,n+1) ds[i] = lower_bound(dis.begin(), dis.end(), s[i]) - dis.begin();
    int res = 0; FTree st;
    rep (i,n) {
        st.modify(ds[i],1);
        res += i+1 - st.query(ds[i+1]); }
    return res; }
