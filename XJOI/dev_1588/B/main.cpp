...
int bsort(double aa[], int n) {
    queue <int> id; while (!id.empty()) { id.pop(); } 
    rep (i,n-1) id.push(i);
    int tot = 0;
    while (!id.empty()) {
        int u = id.front(); id.pop();
        if (aa[u] > aa[u+1]) { 
            if (++tot >= k + 2) { return tot; }
            swap(aa[u], aa[u+1]);
            if (u>0) { id.push(u-1); }
            if (u<n-2) { id.push(u+1); } } }
    return tot; }

double s[N];
int get(double d) {
    s[0] = 0; 
    rep (i,n) { s[i+1] = s[i] + (double)(a[i]) - d; }
    return bsort(s,n+1); }
