...
void init() {
    scanf(III,&n,&k,&q);
    rep (i,k) rep (j,n) scanf(I,&ar[i][j]);
    rep (S,(1<<k)) rep (i,k) if ((S>>i)&1) a[i][S]=1;
}

bool jury(int num, int pos, int now) {
    int S=0;
    rep (i,k) if (ar[i][pos]>=num) S|=(1<<i);
    return a[now][S]; }

void solve() {
    int cnt=k;
    while (q--) {
        int opt,x,y; scanf(III,&opt,&x,&y);
        if (opt==1) { a[cnt++] = a[x-1] | a[y-1]; }
        if (opt==2) { a[cnt++] = a[x-1] & a[y-1]; }
        if (opt==3) {
            int ans=0;
            rep (i,k) if (jury(ar[i][y-1], y-1, x-1)) checkMax(ans, ar[i][y-1]);
            printf(IN,ans); }
    }
}
