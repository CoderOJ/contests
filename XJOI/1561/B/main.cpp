...
void solve() {
    ...
    int m=0; rep (i,n) checkMax(m,a[i]); m=s;
    f[0]=0;
    f[1] = (s-1)*(s-1) %MOD * inv(s)%MOD;
    repa (i,m-1) f[i+1] = ( (2*f[i]-f[i-1]+MOD)%MOD - (s-1+MOD)*inv(s-i)%MOD +MOD ) %MOD;
    rep (i,m+1) see(f[i]);
    int ans=0; rep (i,n) (ans+=f[a[i]]) %= MOD;
    printf (IN,ans);
}
