...
void init() {
    scanf(I,&n);
    int x, y, u;
    while (scanf(III,&x,&y,&u) && x) a[x][y] = u;
}
...
void solve() {
    repi (l,2,n+n) {
        repi (i,1,l-1) repi(j,1,l-1) {
            dp[l][i][j] = max( max(dp[l-1][i][j],dp[l-1][i][j-1]) , max(dp[l-1][i-1][j],dp[l-1][i-1][j-1]));
            dp[l][i][j] += a[i][l-i];
            if (i != j) dp[l][i][j] += a[j][l-j];
        }
    }
    printf(IN, dp[n+n][n][n]);
}
