...
const int N = 105;
const int M = 100005;
int _dp[M*2], *dp = _dp+M;
int _dpn[M*2], *dpn = _dpn+M;
int s[N], f[N];
int n;

void init() {
    scanf(I,&n);
    rep (i,n) scanf(II,&s[i], &f[i]); 
}

void solve() {
    repi (i, -M, M-1) dpn[i] = dp[i] = -0x3f3f3f3f;
    dp[0] = 0;
    rep (i,n) {
        int l = max(-M, -M-s[i]);
        int r = min(M-1, M-1-s[i]);
        repi (j, l, r) dpn[j+s[i]] = max(dp[j+s[i]] , dp[j] + f[i]); 
        repi (j, l, r) if (dpn[j] == -0x3f3f3f3f + f[i]) dpn[j] = -0x3f3f3f3f;
        repi (j, -M, M-1) dp[j] = dpn[j];
    }
    int ans = 0;
    rep (i,M) if (dp[i] >= 0) checkMax(ans, i+dp[i]) ;
    printf(IN,ans);
}
