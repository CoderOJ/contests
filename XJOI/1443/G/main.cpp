...
void init() {
    scanf(II,&n, &m);
    rep (i,n) scanf(III,&v[i], &w[i], &s[i]);    
}

int dp[M];
void getDp(vector < pr > Object) {
    for (auto i: Object) {
        auto v=i.L , w=i.R;
        repb (i, m, v) checkMax(dp[i], dp[i-v] + w);
    }
}
...
void solve() {
    ...
    rep (i,n) {
        int id = 1;
        while (s[i] >= id) {
            Object.push_back( pr(v[i]*id , w[i]*id) );
            s[i] -= id;
            id <<= 1;
        }
        if (s[i]) {
            Object.push_back( pr(v[i]*s[i] , w[i]*s[i]) );
        }
    }
    getDp(Object);
    printf(IN, dp[m]);
