...
int a[N], b[N];
int v[N], r[N];
int _dp[M+M], *dp = _dp+M;
int _dpn[M+M], *dpn = _dpn+M;
int n, k;
...
void solve() {
    for (int i=-M;i<M;i++) dp[i] = dpn[i] = -0x3f3f3f3f;
    dp[0] = 0;
    rep (i,n) {
        int lb = max(-M, -M+v[i]);
        int rb = min(M-1, M-1+v[i]);
        repi (j,lb,rb) {
            dpn[j] = dp[j];
            if (dp[j-v[i]] != -0x3f3f3f3f) checkMax(dpn[j], dp[j-v[i]] + r[i]);
        }
        for (int j=-M;j<M;j++) dp[j] = dpn[j];
    }
    printf(IN, dp[0] <= 0 ? -1 : dp[0]);
}
