...
void init() {
    scanf(II, &n, &m);
    rep (i,n) scanf(I,&a[i]);
    rep (i,m) b[sc.nextInt()-1] ++;
}

int dp[M][M][M][M];
void solve() {
    dp[0][0][0][0] = a[0];
    rep (i0, b[0]+1) rep (i1, b[1]+1) rep (i2, b[2]+1) rep (i3, b[3]+1) {
        int t = i0 + i1*2 + i2*3 + i3*4;
        if (i0) checkMax( dp[i0][i1][i2][i3] , dp[i0-1][i1][i2][i3] + a[t] ) ;
        if (i1) checkMax( dp[i0][i1][i2][i3] , dp[i0][i1-1][i2][i3] + a[t] ) ;
        if (i2) checkMax( dp[i0][i1][i2][i3] , dp[i0][i1][i2-1][i3] + a[t] ) ;
        if (i3) checkMax( dp[i0][i1][i2][i3] , dp[i0][i1][i2][i3-1] + a[t] ) ;
    }
    printf(IN, dp[ b[0] ][ b[1] ][ b[2] ][ b[3] ]);
}
