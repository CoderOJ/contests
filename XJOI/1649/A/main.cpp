...
void solve() {
    dp[0][0][0][0] = 1;
    rep (i,n+1) rep (j,m+1) rep (x,k+1) rep (y,k+1) if (dp[i][j][x][y]) {
        (dp[i+1][j][ max(x,i-j+1) ][ max(y,j-i-1) ] += dp[i][j][x][y]) %= MOD;
        (dp[i][j+1][ max(x,i-j-1) ][ max(y,j+1-i) ] += dp[i][j][x][y]) %= MOD; }
    int ans =0;
    rep (x,k+1) rep (y,k+1) {
        if (x+y <= k) (ans += dp[n][m][x][y]) %= MOD; }
    printf(IN,ans);
}
