...
struct FTree {
    ...
    void modify(int u) {
        if (u>=N) exit(-1);
        u+=2;
        while (u<N) { a[u]++; u+=low(u); } }
    int query(int u) {
        if (u>=N) exit(-1);
        int ans=0; u+=2;
        while (u) { ans += a[u]; u-=low(u); }
        return ans; }
};
...
void init() {
    scanf(I,&n);
    rep (i,n) scanf(I,&a[i]);
    vector <int> ls; rep (i,n) ls.push_back(a[i]);
    sort(ls.begin(), ls.end()); 
    rep (i,n) cnt[i]=i;
    rep (i,n) {
        int u=lower_bound(ls.begin(), ls.end(), a[i]) - ls.begin();
        a[i] = cnt[u]++; }
}
