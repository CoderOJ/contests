...
void preInit() { } void init() {
  ...
  for (int a=2;a<=n;++a) for (int b=1;b<a;++b) {
    for (int k=1;k<a;++k) { F[a][b] = (F[a][b] + 1ll * F[a-b][k] * pow2_kb[k][b]) % p; }
    F[a][b] = 1ll * F[a][b] * binom[a][b] % p * power(2, b*(b-1)/2) % p; }
  for (int a=2;a<=n;++a) for (int b=1;b<a;++b) {
    for (int k=1;k<a;++k) { DF[a][b] = (DF[a][b] + 1ll * (F[a-b][k] + DF[a-b][k]) * pow2_kb[k][b]) % p; }
    DF[a][b] = 1ll * DF[a][b] * binom[a][b] % p * power(2, b*(b-1)/2) % p; }
  for (int a=2;a<=n;++a) for (int b=1;b<a;++b) {
    for (int k=1;k<a;++k) { 
      G[a][b] = (G[a][b] + (G[a-b][k] + 1ll * (F[a-b][k]+DF[a-b][k]) * b) % p * pow2_kb[k][b]) % p; }
