...
void preInit() { } void init() {
    n = sc.nl(); l = sc.nl(); r = sc.nl(); 
    rep (i,n) aa[i] = a[i] = sc.nextInt();
}
...
void solve() {
    ...
    rep (i,n-1) {
        if (a[i]+a[i+1] < l) {
            int del = l - a[i] - a[i+1]; ans += del;
            int can = r-a[i+1]-a[i+2];
            if (can >= del) { a[i+1] += del;}
            else { a[i+1] += can; a[i] += del-can; } } }
    rep (i,n-1) if (a[i]+a[i+1] < l || a[i]+a[i+1] > r) { puts("-1"); return; } 
    printf("%lld\n",ans);
