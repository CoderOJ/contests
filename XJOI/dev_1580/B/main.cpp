...
struct Node { 
    int a,b; 
    Node() { a=b=0; }
    void scan() { a=sc.n(); b=sc.n(); if (a<b) { swap(a,b); } } };
Node a[N];
int n, p;

inline bool check(int a, int b) { return 1LL*a*p <= b*100LL; }
...
inline int allA() {
    int maxScore = a[0].a;
    int ans = 0;
    rep (i,n) if (check(maxScore, a[i].a)) { ans++; }
    return ans; }
...
void solve() {
    ...
    rep (i,n) {
        st.insert(a[i].b); 
        checkMax(preMax, a[i].b);
        int uMax = max(preMax, a[i+1].a);
        while (rMost < n && check(uMax, a[ rMost ].a)) { rMost++; }
        while (!st.empty() && check(uMax, *(--st.end())) ) {
            preChosen++; st.erase( --st.end() ); }
        checkMax(ans, preChosen + rMost - i - 1); }
    printf(IN,ans);
}
