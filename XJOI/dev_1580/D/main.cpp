...
int pack[N];
int get(int u) {
    memset(pack, 0, sizeof(pack)); pack[0] = 1;
    repa (i,n) repb (j,n,1) { (pack[j] += pack[j-1] * f[i][u]) %= p; }
    int res = 0;
    repa (i,n) res += (i*i)%p * (pack[i]-pack[i+1]) % p;
    return res; }

void solve() {
    repa (i,n) { printf("%lld%c", get(i), " \n"[i==n]); }
}
