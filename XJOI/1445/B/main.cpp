...
void solve() {
    ...
    int u0=0 , u1=c[0];
    repa (i,n-1) {
        int v0=0x3f3f3f3f3f3f3f3f , v1=0x3f3f3f3f3f3f3f3f;
        if (u0!=-1) {
            if (s[i] >= s[i-1]) checkMin(v0, u0);
            if (rotate(s[i]) >= s[i-1]) checkMin(v1, u0+c[i]); 
        } 
        if (u1 != -1) {
            if (s[i] >= rotate(s[i-1])) checkMin(v0, u1);
            if (rotate(s[i]) >= rotate(s[i-1])) checkMin(v1, u1+c[i]);
        }
        u0 = v0; u1 = v1;
    }
    int ans = min(u0, u1);
    printf(IN, ans == 0x3f3f3f3f3f3f3f3f ? -1 : ans);
}
