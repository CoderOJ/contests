...
void init() {
    ...
    repa (i,n) {
        string s; int m;
        cin >> s >> m;
        l[i] = (int)s.length() + (m!=0); 
        rep (j,m) { int v; scanf(I,&v); e[i].push_back(v); } }
}

int leaf[N], sum[N];

void dfs0(int u=1) {
    if (e[u].size() == 0) { leaf[u] = 1; sum[u]=0; return; }
    for (int v: e[u]) {
        dfs0(v);
        leaf[u] += leaf[v]; sum[u] += sum[v] + leaf[v] * l[v]; }
}

int ans=0x3f3f3f3f3f3f3f3f;

void dfs1(int u=1, int upSum=0, int upSize=0) {
    if (e[u].size() == 0) return;
    upSum += 3*upSize;

    ses(u);ses(upSum);ses(upSize);ses(sum[u]);see(upSum+sum[u]);
    checkMin(ans, upSum + sum[u]); 

    for (int v:e[u]) {
        dfs1(v, upSum + sum[u] - (sum[v]+l[v]*leaf[v]), upSize+leaf[u]-leaf[v]); } }
