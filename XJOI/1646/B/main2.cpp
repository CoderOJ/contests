...
int sp() {
     ...
     repa (i,n) if (type[i] == 1) { dis[i]=0; que.push({0,i}); }
     while (!que.empty()) {
         pr pos = que.top(); que.pop(); int u=pos.R, uw=pos.L;
         ...
         for (pr p: e[u]) {
            ...
            if (checkMin(dis[v], uw+w)) {
                que.push({dis[v],v}); } } }
     int ans = 0x3f3f3f3f;
     repa (i,n) if (type[i]==2) checkMin(ans,dis[i]);
     return ans; }
...
void solve() {
    ...
    rep (_,15) {
        see(_);
        random_shuffle(ks, ks+k);
        repa (i,n) type[i] = 0;
        rep (i,fs) type[ks[i]] = 1;
        repi (i,fs,k-1) type[ks[i]] = 2;
        checkMin(ans,sp()); }
    printf(IN,ans); 
}
