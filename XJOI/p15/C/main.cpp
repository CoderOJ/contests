...
inline long SPhiu(long n, long c) {
  return (n%MOD) * ((c+1)%MOD) % MOD * (getMu(n/c)%MOD) % MOD; }
...
inline long ansu(long n, long d) {
  return power(getPhi(d), k) * calcSPhi(n / d) % MOD; }
void preInit() { } void init() {
  ...
  for (long i=1; i*i<=n; ++i) {
    if (n % i == 0) {
      (res += ansu(n,i)) %= MOD;
      if (i*i != n) (res += ansu(n, n/i)) %= MOD; } }
  printf("%lld\n", res * (n%MOD) % MOD * power(2,MOD-2) % MOD);
}
