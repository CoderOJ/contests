...
void dfs(int u, int f) {
    col[u] = 1-col[f]; vis[u] = 1;
    for (int v: es[u]) if (v!=f) {
        dfs(v,u); } }
...
bool greedy() {
    A.init(); B.init();
    rep (i,m) {
        if (A.query(e[i].u, e[i].v)) {
            A.merge(e[i].u, e[i].v); }
        else if (B.query(e[i].u, e[i].v)) {
            B.merge(e[i].u, e[i].v); } 
        else { return 0; } }
    A.init(); 
    rep (i,m) {
        if (A.query(e[i].u, e[i].v)) {
            A.merge(e[i].u, e[i].v); 
            es[e[i].u].push_back( e[i].v );
            es[e[i].v].push_back( e[i].u ); } }
    dfs(1,0);
    repa (i,n) printf("%d%c", col[i]+1, " \n"[i==n]); 
    return 1; }
