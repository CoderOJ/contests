...
void dfs(int u, int f) {
    for (int v: e[u]) { if (!col[v]) { que.push(v); } }
    eee.insert( {u,f} ); eee.insert( {f,u} );
    col[u] = 3 - col[f];
    for (int v: me[u]) if (v != f) {
        see("dfs", u , v);
        dfs(v,u); } }
...
bool bfs(int u) {
    ...
    st.init();
    col[u] = 1; 
    for (int v: me[u]) { dfs(v,u); }
    que.push(u);
    while (!que.empty()) {
        int u = que.front(); que.pop();
        for (int v: e[u]) {
            if (col[v]) { 
                if (eee.count( {u,v}) ) { continue; }
                else {
                    see(u,v,st.get(u),st.get(v));
                    if (st.query(u,v)) { return 0; }
                    eee.insert( {u,v} );
                    eee.insert( {v,u} );
                    st.merge(u,v); } }
            else {
                ...
                dfs(v,u);
                que.push(v); } } }
    repa (i,n) printf("%d ",col[i]); 
    return 1; }

void solve() {
    bfs(1); return;
    repa (i,n) if (bfs(1)) { return; }
    puts("-1");
}
