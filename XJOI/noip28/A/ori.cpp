...
{
  int a[B][N][N];
  void init(int _a[][N], int n, int m)
  {
    rep(i, n) rep(j, m) a[0][i][j] = _a[i][j];
    rep(b, B - 1) rep(i, n) rep(j, m)
    {
      int step = 1 << b;
      int& cur = a[b + 1][i][j];
      cur = a[b][i][j];
      if (i + step < n)
        checkMin(cur, a[b][i + step][j]);
      if (j + step < m)
        checkMin(cur, a[b][i][j + step]);
      if (i + step < n && j + step < m)
        checkMin(cur, a[b][i + step][j + step]);
    }
  }
 
  int query(int ux, int uy, int vx, int vy)
  {
    int dx = vx - ux + 1, dy = vy - uy + 1;
    if (dx >= dy) {
      int dl = lg[dy];
      int ds = (1 << dl) - 1;
      int l1 = uy, l2 = vy - ds;
      int res = std::min(a[dl][vx - ds][l1], a[dl][vx - ds][l2]);
      int ul = ux;
      while (ul + ds <= vx) {
        checkMin(res, a[dl][ul][l1]);
        checkMin(res, a[dl][ul][l2]);
        ul += ds + 1;
      }
      return res;
    } else {
      ...
      int ds = (1 << dl) - 1;
      int l1 = ux, l2 = vx - ds;
      int res = std::min(a[dl][l1][vy - ds], a[dl][l2][vy - ds]);
      int ul = uy;
      while (ul + ds <= vy) {
        checkMin(res, a[dl][l1][ul]);
        checkMin(res, a[dl][l2][ul]);
        ul += ds + 1;
      }
      return res;
    }
...
struct LMax
{
  ...
  void init(int _a[][N], int n, int m)
  {
    ...
    rep(b, B - 1) rep(i, n) rep(j, m)
    {
      ...
      cur = a[b][i][j];
      if (i + step < n)
        checkMax(cur, a[b][i + step][j]);
      if (j + step < m)
        checkMax(cur, a[b][i][j + step]);
      if (i + step < n && j + step < m)
        checkMax(cur, a[b][i + step][j + step]);
    }
  }
  ...
  int query(int ux, int uy, int vx, int vy)
  {
    ...
    if (dx >= dy) {
      ...
      int ds = (1 << dl) - 1;
      int l1 = uy, l2 = vy - ds;
      int res = std::max(a[dl][vx - ds][l1], a[dl][vx - ds][l2]);
      int ul = ux;
      while (ul + ds <= vx) {
        checkMax(res, a[dl][ul][l1]);
        checkMax(res, a[dl][ul][l2]);
        ul += ds + 1;
      }
      return res;
    } else {
      ...
      int ds = (1 << dl) - 1;
      int l1 = ux, l2 = vx - ds;
      int res = std::max(a[dl][l1][vy - ds], a[dl][l2][vy - ds]);
      int ul = uy;
      while (ul + ds <= vy) {
        checkMax(res, a[dl][l1][ul]);
        checkMax(res, a[dl][l2][ul]);
        ul += ds + 1;
      }
      return res;
    }
...
struct LSum
{
  ...
  void init(int _a[][N], int n, int m)
  {
    rep(i, n) rep(j, m) a[i + 1][j + 1] = _a[i][j];
    repa(i, n) repa(j, m)
    {
      a[i][j] += a[i - 1][j];
      a[i][j] += a[i][j - 1];
  ...
  int query(int ux, int uy, int vx, int vy)
  {
    return a[vx + 1][vy + 1] - a[ux][vy + 1] - a[vx + 1][uy] + a[ux][uy];
  }
} aSum;
...
init()
{
  n = sc.n();
  m = sc.n();
  rep(i, n) rep(j, m) a[i][j] = sc.nextInt();
  lgInit();
  aMin.init(a, n, m);
  aMax.init(a, n, m);
  aSum.init(a, n, m);
...
solve()
{
  ...
  rep (cs, q) {
    ...
    vy = sc.n();
    if (opt == 'U') {
      printf("%d\n", aSum.query(ux, uy, vx, vy));
    } else if (opt == 'I') {
      printf("%d\n", aMin.query(ux, uy, vx, vy));
    } else {
      printf("%d\n", aMax.query(ux, uy, vx, vy));
    }
  }
}
