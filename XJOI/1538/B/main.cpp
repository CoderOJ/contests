...
struct Node {
    int s; int n; int id;
    bool operator < (Node b) const { return s*b.n==b.s*n ? id < b.id : s*b.n>b.s*n; } };
...
void solve() {
    dfs(R,0); repa (i,n) f[i]=i;
    repa (i,n) p[i] = {w[i],1,i};
    set <Node> st; repa (i,n) if (i!=R) st.insert(p[i]);
    int ans=0; repa (i,n) ans+=p[i].s;
    rep (i,n-1) {
        Node pos=*st.begin(); st.erase(st.begin());
        int l= find(fa[pos.id]); f[pos.id]=l;
        if (l!=R) st.erase(p[l]);
        ans += p[l].n * pos.s;
        p[l].n += pos.n; p[l].s += pos.s;
        if (l!=R) st.insert(p[l]); }
    printf(IN,ans);
}
