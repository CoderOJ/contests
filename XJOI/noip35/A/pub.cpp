...
void solve()
{
  scanf("%s", s);
  n = strlen(s);
  if (n % 2 == 1) { puts(noans); return; }
  int num_le = std::count_if(s, s+n, [](char a) { return a == '('; });
  int num_ri = std::count_if(s, s+n, [](char a) { return a == ')'; });
  int num_any = n - num_le - num_ri;
  if (std::abs(num_le - num_ri) > num_any) { puts(noans); return; }
  int lb = n / 2 - num_le;
  int pos = 0;
  rep (i,n) {
    if (s[i] == '*') {
