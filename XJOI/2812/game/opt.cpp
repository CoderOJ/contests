...
int trans(int a0, int a1, int a2) {
  return trans_rule[a0][a1][a2]; }
...
struct dp_t {
  ...
  bit_t apply(int b) { return bit_t{a[b][0], a[b][1]}; }
  dp_t operator|| (const dp_t b) {
    return dp_t(a[0][0] || b.a[0][0], a[0][1] || b.a[0][1],
                a[1][0] || b.a[1][0], a[1][1] || b.a[1][1]); } 
  dp_t next(int a0, int a1) {
    bit_t pre = apply(a0);
    dp_t next00, next01;
    if (pre.a[0]) next00 = dp_t(trans(0, a1, 0), trans(0, a1, 1));
    if (pre.a[1]) next01 = dp_t(trans(1, a1, 0), trans(1, a1, 1));
    dp_t next1(apply(trans(a0, a1, 0)), apply(trans(a0, a1, 1)));
    return next00 || next01 || next1; }
};
...
struct dpdp_t {
  ...
  dpdp_t operator+ (const dpdp_t b) { dpdp_t r;
    ...
    return r; }
  dpdp_t next(int a0, int a1) { dpdp_t r;
    rep (i0,2) rep(i1,2) rep(i2,2) rep(i3,2)
