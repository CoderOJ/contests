...
void solve() {
    repa (i,n) {
        int c, v; scanf(II,&c,&v);
        vector < int > f;
        rep (j,12) if (c >= (1<<j)) { f.push_back(1<<j); c -= (1<<j); }
        if (c) f.push_back(c);
        repb (j,i,1) {
            bitset < N > u=0;
            repe (k,f) if (f[k] * v <= l) { u |= u<<(f[k]*v); u |= dp[j-1] << (f[k] * v); }
            dp[j] |= u;
        }
    }
    bitset < N > ans;
    scanf(I,&m);
    rep (i,m) ans |= dp[ sc.nextInt() ];
    rep (i,l+1) if (ans[i]) printf("%d ",i);
}
