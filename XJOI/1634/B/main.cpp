...
void solve() {
    dfs(r,0);
    vector <int> le; repa (i,n) if (e[i].size()==1) le.push_back(i);
    sort(le.begin(), le.end(), [](int a,int b) { return dep[a] < dep[b]; });
    int ans = 0;
    for (int u: le) {
        if (mark[u]) continue;
        ses(u);see(dep[u]);
