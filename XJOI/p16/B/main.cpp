...
void preInit() { } void init() {
  ...
  {
    std::multiset<int> st;
    static std::vector<int> def[N];
    rep (i,n) if (l[i] != -1) {
      st.insert(i+1); def[i+1-l[i]].push_back(i+1); }
    rep (i,n) {
      if (st.size()) { checkMin(ans[i], *st.begin() - i); }
      for (int d: def[i]) { st.erase( st.find(d) ); } }
  }
  {
    static std::vector<int> ins[N], ers[N];
    rep (i,n) if (l[i] != -1) {
      slog("[%d, %d): %d", i+1-l[i], i+1, l[i]);
      ins[i+1-l[i]].push_back(l[i]);
      ers[i+1].push_back(l[i]); }
    std::multiset<int> st;
    rep (i,n) {
      for (int d: ins[i]) { st.insert(d); }
      for (int d: ers[i]) { st.erase(st.find(d)); } 
      if (st.size()) { checkMin(ans[i], *st.begin()); } }
  }
  logArray(ans, ans+n);
  rep (i,n) printf("%d\n", ans[i]);
}
