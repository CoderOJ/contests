...
void preInit() { } void init() {
    n=sc.n(); m=sc.n(); p=sc.n(); S=sc.n(); T=sc.n();
    rep (i,m) { int u,v; u=sc.n(); v=sc.n(); e[u][0] |= (1ull<<v); }
}

void solve() {
    repa (step,p-1) {
        repa (i,n) repa (j,n) if (e[i][step-1] & (1ull<<j)) {
            e[i][step] |= e[j][step-1]; } }
    static Mask fe[N];
    repa (i,n) rep (j,p) fe[i] |= e[i][j];
