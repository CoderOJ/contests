...
int spare[N], require[N];


void preInit() { } void init() {
    n=sc.n(); x=sc.n(); y=sc.n();
    rep (i,n) a[i] = sc.n();
    rep (i,n) {
        spare[i] = (a[i]+y-1) / y;
        int ado = a[i] - (spare[i]-1) * y;
        require[i] = (spare[i]-1) - (ado+x-1) / x; 
        see( spare[i], require[i] );
    }
}
...
void solve() {
    ...
    rep (i,n) {
        ...
        if (pre + require[i] >= 0) {
            ans ++; 
            pre += require[i];
            chosen.push( spare[i] - require[i] ); }
        else {
            if (chosen.size()!=0 && pre + chosen.top() + require[i] >= 0 && pre + chosen.top() + require[i] > pre + spare[i]) {
                pre += chosen.top() + require[i];
                chosen.pop(); chosen.push( spare[i] - require[i] ); } 
            else {
                pre += spare[i]; }
        }
    }
