...
void init() {
    memset(c,-1,sizeof(c)); memset(dp,0,sizeof(dp));
    scanf(II,&n,&k);
    rep (i,n-1) { int u,v; scanf(II,&u,&v); e[u].push_back(v); e[v].push_back(u); }
    rep (i,k) { int b,_c; scanf(II,&b,&_c); c[b] = _c-1; }
}

void dfs(int u, int f) {
    for (int v: e[u]) if (v!=f) dfs(v,u);
    rep (cc,3) if (cc == c[u] || c[u] == -1) {
        dp[u][cc] = 1;
        for (int v:e[u]) if (v!=f) {
            int tot=0; rep (i,3) if (i!=cc) tot+=dp[v][i];
            (dp[u][cc] *= tot) %= MOD; } } 
}

void solve() {
    dfs(1,0);
    printf(IN, (dp[1][0] + dp[1][1] + dp[1][2]) % MOD);
}
