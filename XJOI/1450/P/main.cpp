...
void init() {
    scanf(I,&m); scanf("%s",fir); scanf("%s",bac); 
    n = strlen(fir);
}
...
inline int getw(char* s, char u) {
    rep (i,n) if (s[i] == u) return i; 
    return -1;
}

int decode (int la, int ra, int lb, int rb) {
    int res = 1;
    int sta = la+1, stb=lb, cnt = 0;
    while (sta <= ra) {
        int edb = getw(bac, fir[sta]);
        int eda = sta + edb - stb;
        ses(sta);ses(eda);ses(stb);see(edb);
        cnt++;
        res *= decode(sta, eda, stb, edb); 
        sta = eda + 1;
        stb = edb + 1;
    }
    return res * nCr(m, cnt);
}

void solve() {
    see("test");
    printf(IN, decode(0, n-1, 0, n-1));
}
