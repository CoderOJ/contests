...
char decode(int l, int r, int ll, int rr) {
    ses(l);ses(r);ses(ll);see(rr);
    if (l > r) return 0;
    char u = bac[r];
    int split; repi (i,ll,rr) if (mid[i] == u) split = i;
    int l0 = split - ll , l1 = rr - split;
    ls[(int)u] = decode(l, l+l0-1, ll, ll+l0-1);
    rs[(int)u] = decode(r-l1, r-1, rr-l1+1, rr);
    return u;
}
