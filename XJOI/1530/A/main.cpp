...
void init() {
    scanf(III,&n,&m,&k); rep (i,m) scanf(I,&a[i]); rep (i,k) scanf(III,&t[i],&x[i],&y[i]);
    rep (i,m) s[cc++]=a[i]; rep (i,k) if (t[i]==1) s[cc++]=y[i]; sort(s,s+cc); cc=unique(s,s+cc)-s;
    rep (i,m) a[i]=lower_bound(s,s+cc,a[i])-s; rep (i,k) if (t[i]==1) y[i]=lower_bound(s,s+cc,y[i])-s;
}
int bcnt;
...
int query1(int u) {
    ...
    see(completeBars);
    repi (i,startPoint,u) insert(c[completeBars], f[completeBars], a[i]);
        rep (i,5) ses(c[completeBars][i]); see(""); 
        rep (i,5) ses(f[completeBars][i]); see(""); 
