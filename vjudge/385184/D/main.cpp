...
bool checkValid(int u) {
    int res=0;
    rep (i,u) {
        for (int j=i;j<n;j+=u) if (s[j]=='P') goto nxt; 
        res++; 
        nxt:; }
    return res; }

void solve() {
    vector <int> fc; fc.push_back(1);
    for (int i=2;i*i<=n;i++) {
        if (n%i==0) {
            fc.push_back(i);
            if (i*i!=n) fc.push_back(n/i); } } 
    int ans=0; 
    for (int i: fc) { ses(i); ans += checkValid(i)*count(i); see(ans); }
    printf(IN,ans);
}
