...
void solve() {
    int ans=0x3f3f3f3f;
    rep (l,n) {
        memset (dp,0x3f3f3f3f,sizeof(dp)); int r=l+n;
        repi (i,l,r-1) {
            int a1=t1, a2=t2;
            { int ls=lower_bound(p,p+n+n,p[i]-t1)-p-1; see(ls); if(ls>=l) a1+=dp[ls]; }
            { int ls=lower_bound(p,p+n+n,p[i]-t2)-p-1; see(ls); if(ls>=l) a2+=dp[ls]; }
            dp[i]= min(a1,a2); }
        checkMin(ans, dp[r-1]); }
    printf(IN,ans);
}
