...
void init() {
    scanf(I,&n); if (n==0) throw (bool)(1);
    rep (i,n) scanf(I, &a[i]);
    rep (i,n) a[i+n] = a[i];    
}

void solve() {
    rep (i,n*2) dp[i][i+1]=0;
    repi (l,2,n) for (int i=0;i+l<=n*2;i++) {
        int j=i+l; dp[i][j]=0x3f3f3f;
        repi (k,i+1,j-1) checkMin(dp[i][j], dp[i][k] + dp[k][j] + __gcd(a[i], a[j])); }
    int ans=0x3f3f3f3f;
    rep (i,n) repi (j,i+1,i+n-1) checkMin(ans, dp[i][j] + dp[j][i+n] + __gcd(a[i],a[j]));
    printf(IN,ans); }
