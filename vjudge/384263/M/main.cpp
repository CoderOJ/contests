...
void initTs(int u, int s) {
    if (u==n) { if (s==m) ts.push_back(tts); return; }
    rep (i,a[u]+1) if (s+i<=m) {
        tts.push_back(i); initTs(u+1,s+i); tts.pop_back(); } }

void init() {
    if (scanf(II,&n,&m)!=2) throw (bool)1;
    rep (i,n) scanf(I,&a[i]); 
    ts.clear(); tts.clear();
    initTs(0,0);
}
