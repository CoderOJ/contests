...
void init() {
    scanf(I,&n); scanf("%s",s+1); s[0]='.'; s[n+1]='.';
}
...
bool valid(int u) {
    ...
    while (s[l] == 'W') l--;
    while (s[r] == 'W') r++;
    if (s[l] == 'B' && s[r] == 'B') return 0;
    return 1; }

int get(int u) {
    int ans=0, l=u-1, r=u+1;
    while (s[l] == 'B') l--;
    while (s[r] == 'B') r++;
    if (s[l] == 'W') checkMax(ans, u-l-1);
    if (s[r] == 'W') checkMax(ans, r-u-1); 
    return ans; }

void solve() {
    int ans=0;
    repa (i,n) { if (s[i] == '.') if (valid(i)) checkMax(ans,get(i)); ses(i);see(ans); } 
    printf(IN,ans);
}
