...
struct Tr : public Tree::TreeLca<N> {
    int w[N], mark[N];
    void dfs(int u, int f) {
        w[u] += w[f];
        auto vs = this->getNodes(u);
        for (int v: vs) if (v!=f) { dfs(v,u); } }
    int solve() {
        vector <int> ord;
        repa (i,n) { ord.push_back(i); }
        sort(ord.begin(), ord.end(), [this](int a, int b) { return this->d[a] > this->d[b]; }); 
        int ans = 0;
        for (int u: ord) {
            if (mark[u]) { continue; }
            int pl=0, ps=0, p = u;
            repb (i,this->B-1,0) { 
                ...
                int vl = pl+d[u]-d[v];
                int vs = ps+w[u]-w[v];
                if (vl<=l && vs<=s) { u = v; ps=vs; pl=vl; } }
            if (u == p) { return -1; }
            ans++; while (p!=u) { mark[p]=1; p=fa[p][0]; } }
        return ans; }
};
...
void preInit() { } void init() {
    n=sc.n(); l=sc.n(); s=sc.n();
    repa (i,n) T.w[i] = sc.n(); 
    repi (i,2,n) T.addEdge(sc.n(), i); 
}
